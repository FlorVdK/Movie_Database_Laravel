...
@section('content')
<p class="breadcrumb"><a href="{{ url('/') }}/directors">Directors</a> </p>
<div class="sort_bar">
            <a href="/?genres">Generes</a> |
            {!! Form::open(array('url' => 'director/sort', 'method' => 'GET')) !!}
            {!! Form::select('sort', array('1' => 'Sort By', '2' => 'Release &darr;', '3' => 'Release &uarr;', '4' => 'Alphabet'), '', array('onchange' => 'this.form.submit()')) !!}
            {{ Form::hidden('invisible', $director->id , array('id' => $director->id  )) }}
            {!! Form::close() !!}
        </div>
    @if ($movies)
